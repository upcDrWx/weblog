...
import com.wx.weblog.admin.convert.ArticleDetailConvert;
...
public class AdminArticleServiceImpl implements AdminArticleService {
    ...
    private void insertTags(Long articleId, List<String> publishTags) {
        // 筛选提交的标签（表中不存在的标签）[新的标签1，新的标签2]
        List<String> notExistTags = null;
        // 筛选提交的标签（表中已存在的标签）[1，2]
        ...
        if (CollectionUtils.isEmpty(tagDOS)) {
            ...
            // 补充逻辑：
            // 还有一种可能：按字符串名称提交上来的标签，也有可能是表中已存在的，
            // 比如表中已经有了 Java 标签，用户提交了个 java 小写的标签，需要内部装换为 Java 标签
    ...
    public Response updateArticle(UpdateArticleReqVO updateArticleReqVO) {
        ...
        CategoryDO categoryDO = categoryMapper.selectById(categoryId);
        if (Objects.isNull(categoryDO)) {
            log.warn("==> 提交的分类不存在，categoryId: {}", categoryId);
            throw new BizException(ResponseCodeEnum.CATEGORY_NOT_EXISTED);
        }
        ...
        // 4. 更新文章标签
        articleTagRelMapper.deleteByArticleId(articleId);
        insertTags(articleId, updateArticleReqVO.getTags());
